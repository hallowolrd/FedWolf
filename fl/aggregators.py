...
FEDWOLF_FUSION_MODE_EVIDENCE_FILTER_BLOCK_PRECISION = "evidence_filter_block_precision"
FEDWOLF_FUSION_MODE_ROBUST_UPDATE_FUSION = "robust_update_fusion"
FEDWOLF_FUSION_MODES = (
    FEDWOLF_FUSION_MODE_EVIDENCE_FILTER_BLOCK_PRECISION,
    ...
    *,
    ...
    if variant in {
        ...
        has_precision_field = any(
            ...
            and "expert_block_fisher_precision_by_layer" in client_stat
            for client_stat in client_stats
        )
        if not has_precision_field:
            raise RuntimeError(
                f"{variant} requires client_stats['expert_block_fisher_precision_by_layer'], "
        ...
        for key in client_updates[0].keys():
            ...
            for client_idx, update in enumerate(client_updates):
                ...
                if variant == FEDWOLF_UPDATE_FUSION_VARIANT_FISHER_WOLF:
                    ...
                    precision, precision_error = _get_client_block_fisher_precision(
...
def build_aggregator(args):
    ...
    if args.agg_method == "fedwolf":
        ...
        if fusion_mode == FEDWOLF_FUSION_MODE_EVIDENCE_FILTER_BLOCK_PRECISION:
        ...
        raise ValueError(
            f"Unknown fedwolf_fusion_mode: {fusion_mode}. "
            f"Expected one of {list(FEDWOLF_FUSION_MODES)}."
        )
